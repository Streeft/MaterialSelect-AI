...
const kinds: SynthesisKindInfo[] = [
  ...
  },
  {
    ...
    label: "Painel sanduíche (faces e núcleo)",
    note: "Módulo equivalente pela inércia das faces e do núcleo.",
    rules: {
      densidade: {
      ...
      modulo_young: {
        key: "sandwich-modulo",
        label: "Módulo de flexão equivalente",
        formula: "Eeq = Ef·[1 - (c/h)³] + Ec·(c/h)³",
        basis: "exato",
        basis_label: "Exata",
      },
    },
    without_rule: {
      limite_escoamento: "A resistência depende do vão e do modo de falha.",
    },
  },
];
...
describe("Sintetizar material", () => {
  ...
  it("troca os campos ao escolher painel sanduíche e envia as espessuras", async () => {
    const user = userEvent.setup();
    await open();
    ...
    expect(
      await screen.findByShadowLabelText(/Espessura de cada face/),
    ).toBeInTheDocument();
    expect(
      await screen.findByShadowLabelText(/Espessura do núcleo/),
    ).toBeInTheDocument();
    expect(
      await screen.findByShadowRole("combobox", { name: t.parentBCoreLabel }),
    ).toBeInTheDocument();

    await runPreview(user);

    const sent = previewSynthesis.mock.calls[0]![0]!;
    expect(sent.kind).toBe("painel_sanduiche");
    expect(sent.face_thickness_mm).toBe(1.0);
    expect(sent.core_thickness_mm).toBe(10.0);
    expect(sent.parent_b_id).toBe(2);
    expect(sent.volume_fraction).toBeUndefined();
    expect(sent.relative_density).toBeUndefined();
  });
