...
from app.calculations.synthesis import COMPOSITO, ESPUMA, PAINEL_SANDUICHE
...
class SynthesisRequest(BaseModel):
    """A receita. Os campos de outros tipos são recusados, nunca ignorados."""

    kind: Literal[COMPOSITO, ESPUMA, PAINEL_SANDUICHE]
    ...
    #: Painel sanduíche: ``parent_a_id`` (faces), ``parent_b_id`` (núcleo), e espessuras.
    parent_a_id: int
    ...
    face_thickness_mm: float | None = Field(default=None, gt=0.0)
    core_thickness_mm: float | None = Field(default=None, gt=0.0)

    @field_validator("volume_fraction", "relative_density", "face_thickness_mm", "core_thickness_mm")
    @classmethod
