...
def _face(**overrides) -> dict[str, ParentValue]:
    values = {
        DENSIDADE: _p(_RHO_FACE),
        "modulo_young": _p(_E_FACE),
        "custo_massa": _p(_CUSTO_FACE),
        "condutividade_termica": _p(_K_FACE),
        "temp_max_servico": _p(420.0),
        "energia_incorporada": _p(200.0),
        "pegada_co2": _p(12.0),
        "energia_reciclagem": _p(25.0),
        "co2_reciclagem": _p(1.5),
        "resistencia_tracao": _p(310.0e6),
        "limite_escoamento": _p(270.0e6),
        "dureza": _p(95.0),
    }
    values.update(overrides)
    return values
...
def _core(**overrides) -> dict[str, ParentValue]:
    values = {
        DENSIDADE: _p(_RHO_CORE),
        "modulo_young": _p(_E_CORE),
        "custo_massa": _p(_CUSTO_CORE),
        "condutividade_termica": _p(_K_CORE),
        "temp_max_servico": _p(100.0),
        "energia_incorporada": _p(100.0),
        "pegada_co2": _p(5.0),
        "energia_reciclagem": _p(15.0),
        "co2_reciclagem": _p(0.8),
        "resistencia_tracao": _p(1.5e6),
        "limite_escoamento": _p(1.2e6),
        "dureza": _p(10.0),
    }
    values.update(overrides)
    return values
...
# --- painel sanduíche (faces e núcleo) -------------------------------------


def test_painel_sanduiche_densidade_equivalente() -> None:
    """Conservação de massa em volume simétrico 2t + c."""
    result = synthesize_sandwich(
        face_thickness_mm=1.0, core_thickness_mm=10.0, face=_face(), core=_core()
    )
    densidade = _by_slug(result)[DENSIDADE]

    # h = 12 mm, 2t*rho_f + c*rho_c = 2*2700 + 10*100 = 6400 kg·mm/m³
    esperado = (2.0 * 1.0 * _RHO_FACE + 10.0 * _RHO_CORE) / 12.0
    assert densidade.value == pytest.approx(esperado)
    assert densidade.rule.basis == EXATO


def test_painel_sanduiche_modulo_flexao_equivalente() -> None:
    """Módulo equivalente derivado da inércia homogeneizada da viga sanduíche."""
    result = synthesize_sandwich(
        face_thickness_mm=1.0, core_thickness_mm=10.0, face=_face(), core=_core()
    )
    modulo = _by_slug(result)["modulo_young"]

    # h = 12, c/h = 10/12 = 5/6, (c/h)^3 = 125/216
    ratio_cubed = (10.0 / 12.0) ** 3
    esperado = _E_FACE * (1.0 - ratio_cubed) + _E_CORE * ratio_cubed

    assert modulo.value == pytest.approx(esperado)
    assert modulo.rule.basis == EXATO


def test_painel_sanduiche_custo_ponderado_por_massa() -> None:
    """Custo pondera por fração mássica: faces pesadas carregam a maior parte."""
    result = synthesize_sandwich(
        face_thickness_mm=1.0, core_thickness_mm=10.0, face=_face(), core=_core()
    )
    custo = _by_slug(result)["custo_massa"]

    # m_f = 2*1*2700 = 5400, m_c = 10*100 = 1000, m_tot = 6400
    w_f = 5400.0 / 6400.0
    w_c = 1000.0 / 6400.0
    esperado = w_f * _CUSTO_FACE + w_c * _CUSTO_CORE

    assert custo.value == pytest.approx(esperado)
    # Por volume daria (2/12)*3.5 + (10/12)*5.0 = 4.75; por massa dá ~3.73
    assert custo.value != pytest.approx((2.0 / 12.0) * _CUSTO_FACE + (10.0 / 12.0) * _CUSTO_CORE)


def test_painel_sanduiche_condutividade_termica_transversal_em_serie() -> None:
    """Fluxo transversal atravessa as três camadas em série: resistências somam."""
    result = synthesize_sandwich(
        face_thickness_mm=1.0, core_thickness_mm=10.0, face=_face(), core=_core()
    )
    cond = _by_slug(result)["condutividade_termica"]

    # R_tot = 2*1.0/200 + 10.0/0.03, k_eq = 12.0 / R_tot
    r_tot = (2.0 * 1.0 / _K_FACE) + (10.0 / _K_CORE)
    esperado = 12.0 / r_tot

    assert cond.value == pytest.approx(esperado)
    assert cond.rule.basis == EXATO


def test_painel_sanduiche_temperatura_maxima_e_o_minimo() -> None:
    result = synthesize_sandwich(
        face_thickness_mm=1.0, core_thickness_mm=10.0, face=_face(), core=_core()
    )
    t_max = _by_slug(result)["temp_max_servico"]
    assert t_max.value == pytest.approx(100.0)


def test_painel_sanduiche_resistencia_e_dureza_ficam_de_fora_com_motivo() -> None:
    result = synthesize_sandwich(
        face_thickness_mm=1.0, core_thickness_mm=10.0, face=_face(), core=_core()
    )
    ...
    assert "dureza" in skipped
    assert "vão" in skipped["limite_escoamento"] or "falha" in skipped["limite_escoamento"]


def test_painel_sanduiche_qualidade_propaga_pior() -> None:
    result = synthesize_sandwich(
        face_thickness_mm=1.0,
        core_thickness_mm=10.0,
        face=_face(densidade=_p(_RHO_FACE, DataQuality.MEDIDO)),
        core=_core(densidade=_p(_RHO_CORE, DataQuality.IMPORTADO)),
    )
    assert _by_slug(result)[DENSIDADE].quality == DataQuality.IMPORTADO


def test_painel_sanduiche_espessuras_invalidas_sao_recusadas() -> None:
    with pytest.raises(SynthesisError, match="positivas"):
        synthesize_sandwich(
            face_thickness_mm=-1.0, core_thickness_mm=10.0, face=_face(), core=_core()
        )
    with pytest.raises(SynthesisError, match="positivas"):
        synthesize_sandwich(
            face_thickness_mm=1.0, core_thickness_mm=0.0, face=_face(), core=_core()
        )
    with pytest.raises(SynthesisError, match="finitos"):
        synthesize_sandwich(
            face_thickness_mm=float("nan"), core_thickness_mm=10.0, face=_face(), core=_core()
        )
...
@pytest.mark.parametrize("kind", [COMPOSITO, ESPUMA, PAINEL_SANDUICHE])
