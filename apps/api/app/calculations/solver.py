...
from app.calculations.load_cases import LoadCase
...
class SolvedRecord:
    ...
    index_value: float
    #: The objective — mass, in ``case.objective_unit``.
...
def solve(
    ...
    *,
    ...
    Args:
        ...
        index_expression: the **catalogued** expression for ``case.index_slug``.
        ...
        limit: how many solved records to return, lightest first.
        ...
        if evaluation.value <= 0:
            excluded.append(
                ExcludedRecord(
                    ...
                    name=name,
                    missing_keys=[],
                    reason="Índice não positivo: a massa seria negativa ou infinita.",
                )
            )
            continue
        try:
