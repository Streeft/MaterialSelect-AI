...
async def upload_file(
    ...
    return await run_in_threadpool(ImportService(db, user).upload, file.filename or "arquivo", data)
...
def preview(
    ...
    return ImportService(db, user).preview_sheet(job_id, payload.sheet_name)
...
def rollback(
    ...
    return ImportService(db, user).rollback(job_id)
...
def list_jobs(
    ...
    return ImportService(db, user).list_jobs()
...
def get_report(
    ...
    return ImportService(db, user).get_report(job_id)
...
def list_templates(
    ...
    return ImportService(db, user).list_templates()
...
def create_template(
    ...
    return ImportService(db, user).create_template(payload)
