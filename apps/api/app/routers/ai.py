...
def ai_status(db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> AIStatusOut:
    ...
    return AIService(db, viewer_id=user.id).status()
...
def interpret(
    ...
    return AIService(db, viewer_id=user.id).interpret(payload)
...
def explain(
    ...
    return AIService(db, viewer_id=user.id).explain(payload.study_id, project.id)
