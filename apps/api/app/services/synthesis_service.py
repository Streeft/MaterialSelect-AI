...
class SynthesisService:
    ...
    def _run(self, request: SynthesisRequest) -> tuple[SynthesisResult, list[Material]]:
        ...
        if request.kind == COMPOSITO:
            ...
            if request.face_thickness_mm is not None or request.core_thickness_mm is not None:
                raise ValidationError(
                    "Espessuras de face e núcleo são campos de painel sanduíche e não entram num compósito."
                )
        ...
        if request.kind == ESPUMA:
            ...
            if request.face_thickness_mm is not None or request.core_thickness_mm is not None:
                raise ValidationError(
                    "Espessuras de face e núcleo são campos de painel sanduíche e não entram numa espuma."
                )
        ...
        if request.kind == PAINEL_SANDUICHE:
            if request.parent_b_id is None:
                raise ValidationError(
                    "Um painel sanduíche precisa de material para as faces e para o núcleo."
                )
            if request.volume_fraction is not None:
                raise ValidationError(
                    "Fração volumétrica é campo de compósito e não entra num painel sanduíche."
                )
            if request.relative_density is not None:
                raise ValidationError(
                    "Densidade relativa é campo de espuma e não entra num painel sanduíche."
                )
            if request.face_thickness_mm is None or request.core_thickness_mm is None:
                raise ValidationError("Informe a espessura de cada face e do núcleo em mm.")
            if request.parent_b_id == request.parent_a_id:
                raise ValidationError(
                    "As faces e o núcleo são o mesmo material; o resultado seria uma chapa maciça dele."
                )
            core = self._parent(request.parent_b_id)
            try:
                result = synthesize_sandwich(
                    ...
                    core=self._values(core),
                )
            except SynthesisError as exc:
                raise ValidationError(str(exc)) from exc
            return result, [first, core]

        raise ValidationError(f"Tipo de síntese desconhecido: {request.kind}")
...
__all__ = ["SynthesisService", "COMPOSITO", "ESPUMA", "PAINEL_SANDUICHE"]
