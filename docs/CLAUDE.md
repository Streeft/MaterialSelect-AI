...
- **Ninguém entra sem concessão.** O portão de D-46 não tem exceção e o Stripe
  ...
  A exceção deliberada é o **modo aberto** ([D-83](DECISIONS.md)), para uma
...
| `resolve.conditions` do `vitest.config.ts` deixando de valer | Desde o Vite 6 o pipeline de SSR — que é por onde o Vitest resolve `node_modules` — lê **as suas próprias** condições. Repita a condição em `ssr.resolve.conditions` e `ssr.resolve.externalConditions`; as listas têm de concordar. Sem isso o `lit-html` cai no build `node/` (`isServer` fixo em `true`), a delegação de ARIA do `@material/web` desliga em silêncio e nove testes de shadow DOM falham. |
| `npm audit fix` num monorepo de workspaces | Não use a versão cega. Ela re-hoista pacotes de forma assimétrica — já mandou o `vitest` para a raiz deixando o `jsdom` em `apps/web`, e o vitest resolve o jsdom a partir da própria localização: 24 arquivos de teste morreram com `Cannot find package 'jsdom'`. Declare as versões no `package.json` e deixe o npm re-resolver a árvore inteira. |
| `fixAvailable` do `npm audit` apontando versão **menor** | É downgrade de major, não correção: a versão sugerida é anterior à que introduziu a dependência vulnerável. Já aconteceu com `@lhci/cli@0.1.0` e com `plotly.js@2.34.0`. Confira o número antes de seguir a sugestão. |
| Lockfile sem `resolved`/`integrity` na maioria das entradas | O `npm audit` não consegue identificar esses pacotes e **subconta** — o lockfile antigo tinha 59 de 1095 entradas completas e escondia dois críticos. Um `npm audit` só é comparável entre máquinas com o lockfile completo. |
| Campo **obrigatório** no esquema de um provedor em modo `schema` que o prompt nem sempre torna respondível | O servidor (Groq) valida o JSON gerado e descarta a resposta inteira por um campo ausente. Monte o esquema por contexto, como `explain_schema`/`interpret_schema`: o campo só existe quando a pergunta existe ([D-89](DECISIONS.md)). |
| Pôr "soma dos pesos = 1" no `RunRequest` | Não. O laudo e "Executar" em "Meus estudos" reexecutam estudos salvos com pesos 1, 1, 1; o `/run` renormaliza e a regra vale na tela ([D-87](DECISIONS.md)). |
| Debounce de um objeto de requisição | Faça o debounce da **chave serializada** (`JSON.stringify`), nunca do objeto: um objeto novo a cada render é uma requisição nova a cada render. |
| `Tabs` e um id de seção que "sumiu" | `Tabs` renderiza só o painel ativo; um `#excluidos` só existe na aba Eliminados. O `ResultsView` abre a aba pelo hash ao montar ([D-85](DECISIONS.md)). |
