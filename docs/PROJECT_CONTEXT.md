...
documento passou a publicar também o **nível médio** — **3,03**, que cruza 3 pela
primeira vez; era 2,84 depois do P2, 2,56 depois
do P1-4, 2,44 depois
...
caiu sobre as duas capacidades **mais atrasadas da tabela** — uma delas a única
em zero — e quase tudo que precisou já existia no backend desde o P0-2 e o P0-4.

**A faixa P1 fechou com o P1-4** (`My Records` 0→3, Browse 4→5), e a leitura dele
é o contrário da do P1-3: aquele foi apresentação sobre modelo pronto, este teve
de abrir uma fronteira que não existia. Um registro que pertence a uma pessoa
...
sistema, e decidir **em que espaço** medi-la é escolha de método.

**O P2 restante fechou a faixa** ([D-64](DECISIONS.md)), movendo as duas últimas
linhas que estavam em **zero** (Engineering Solver 0→3, Performance Index Finder
0→3) e levando a média acima de 3. O achado do desenho é que os dois itens são
**uma derivação só** lida de dois jeitos, e a forma que isso tomou —
`massa = fator estrutural / índice`, com o índice lido do catálogo e nunca
reescrito ao lado — é a mesma recusa de duas verdades que o D-60 e o D-63 já
...
**Saúde do código:** 1856 testes de backend (Python 3.11 e 3.12, nenhum skip)
e 505 de frontend, todos verdes. Desde o P0-1 a suíte também roda as migrações de
...
|---|---|---|---|
| Maior *chunk* de JavaScript | 4,5 MB | **981 KB** (−79%) | `next build`; o Plotly completo era 79% de todo o JS da aplicação. Desde D-80 o chunk do Plotly é **865 KB** (864.731 B): só `scatter` continua registrado, as demais figuras são SVG próprio. |
