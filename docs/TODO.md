...
Nenhum item aberto no momento — **B11** foi quitado nesta sessão (ver "Débitos já quitados").
...
- ~~**B11** — a unidade canônica era impressa como o Pint a escreve~~ — `pretty_unit`
  ...
  (`³`, `²`), potências arbitrárias em `^`, multiplicações `*` em pontos medianos
  (`·`), e `dimensionless` em traço `—`. Integrado em `app/services/export_service.py`
  em todos os eixos de gráficos (`_figure_axis`), proveniência de materiais, proveniência
  de processos e catálogo. Coberto por testes unitários dedicados em `test_units.py` e
  regressão no exportador do catálogo em `test_exports.py`.
...
- ~~**P3 (terceiro item)** — o Synthesizer~~ — `GET /api/sintetizar/tipos`,
  `POST /api/sintetizar/previa` e `POST /api/sintetizar`, mais `/app/sintetizar`
  ([D-67](DECISIONS.md)). Gera materiais hipotéticos (compósitos e espumas) a partir
  de materiais catalogados com receita auditável. A regra é da propriedade:
  densidade por volume (exato), módulo em compósito como par de limites Voigt-Reuss,
  grandezas específicas por fração mássica, temperatura máxima pelo mínimo, e
  Gibson-Ashby para espumas celulares. Resistência excluída honestamente em
  compósitos com motivo técnico explícito (`_NO_RULE`). Propagação da pior qualidade
  dos pais. Registros sempre isolados no espaço do usuário (`owner_id NOT NULL`,
  restrição de banco `ck_material_sintetizado_tem_dono`) com receita persistida em
  `material_synthesis`.
