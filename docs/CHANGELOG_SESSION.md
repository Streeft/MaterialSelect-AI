...
**O achado do desenho do Módulo R.** Um painel sanduíche simétrico não é uma mistura homogênea volumétrica arbitrária: sua finalidade de projeto é rigidez estrutural à flexão com massa mínima. O módulo de flexão equivalente homogeneizado $E_{eq}$ decorre da teoria de vigas/placas sanduíche ($D = E_{eq} \frac{b h^3}{12}$), resultando em:
$$E_{eq} = E_f \left[1 - \left(\frac{c}{h}\right)^3\right] + E_c \left(\frac{c}{h}\right)^3$$
onde $t$ é a espessura de cada face, $c$ a espessura do núcleo e $h = 2t + c$ a espessura total.

**Propriedades mássicas e de transporte.** Densidade aparente equivalente $\rho_{eq} = (2t\rho_f + c\rho_c)/h$ é exata. Propriedades por massa (custo, energia e carbono) misturam pela fração mássica $w_f = 2t\rho_f / (h\rho_{eq})$ e $w_c = c\rho_c / (h\rho_{eq})$. A condutividade térmica transversal em série segue a soma das resistências térmicas:
$$R_{tot} = \frac{2t}{k_f} + \frac{c}{k_c} \implies k_{eq} = \frac{h}{R_{tot}}$$
A temperatura máxima de serviço é o mínimo dos dois materiais ($\min(T_{max, f}, T_{max, c})$).

**Omissões auditadas em `_NO_RULE`.** Resistência mecânica e dureza não possuem lei analítica geral sem caracterização de modos de falha (escoamento da face, enrugamento/wrinkling, cisalhamento do núcleo ou delaminação). Em estrito cumprimento ao Princípio 3 (dados ausentes nunca viram zero), são omitidas com a respectiva justificativa técnica documentada.

**Quitação do débito B11.** O formatador `pretty_unit` foi adicionado a `app/calculations/units.py`, espelhando `prettyUnit` do frontend. Ele substitui potências `** 3` e `** 2` por sobrescritos (`³`, `²`), potências arbitrárias por `^`, multiplicações `*` por pontos medianos (`·`) e `dimensionless` por traço `—`. Foi integrado em `export_service.py` para eixos de gráficos em SVG (`_figure_axis`), proveniência de materiais, proveniência de processos e exportações de catálogo.

**Maturidade da plataforma.** Sandwich Panels subiu de 0 para **4** e Unidades de exibição de 2 para **3**. A cobertura de capacidades do EduPack subiu de 91% para **~97%** (31 de 32 capacidades funcionais), com nível médio de **3,53**. Faixas P1, P2 e P3 100% concluídas.

---

## Sessão 22 — 15/09/26 — P3: o Synthesizer (compósitos e espumas)

**O pedido.** "Continue o projeto", iniciando a implementação da faixa P3 pendente — o Synthesizer (Módulo Q).

**O achado do desenho.** O Synthesizer é o módulo que mais perto passa de "inventar propriedade de material" (violação do princípio 1). A resposta arquitetural é que **um valor sintetizado não é inventado: é calculado e carrega a derivação auditável**. Cada número gerado nomeia a lei física que o produziu e sua base epistêmica (`exato`, `limites` ou `empirico`).

**A regra é da propriedade, não da receita.** Aplicar regra das misturas indistintamente a qualquer número é a forma de mentir sem errar a aritmética. Densidade mistura linearmente por volume (conservação de massa estrita). Módulo de Young em compósito vira **par de limites** (Voigt e Reuss) porque depende da orientação ausente no catálogo. Propriedades por unidade de massa misturam por **fração mássica** (dependendo da densidade de cada constituinte). Temperatura máxima de serviço é o **mínimo** dos constituintes. E **resistência de compósito não tem regra**: depende da interface fibra-matriz; propriedades sem lei honesta são omitidas com motivo técnico registrado (`_NO_RULE`), respeitando D-24 e princípio 3.

**Espumas e a mecânica celular.** Seguem Gibson–Ashby (célula aberta): módulo escala com $R^2$ e colapso plástico com $0,3 \cdot R^{3/2} \cdot \sigma_s$. Propriedades por massa são herdadas do sólido.

**Qualidade do dado e isolamento.** Propagação da **pior qualidade** dos pais (`MEDIDO` > `IMPORTADO` > `ESTIMADO`). Todo registro derivado pertence ao usuário (`owner_id NOT NULL`, verificado por `ck_material_sintetizado_tem_dono`), nunca poluindo o catálogo compartilhado. A receita completa reside em `material_synthesis`.

**Verificação.** 57 testes novos de backend (`test_synthesis.py`, `test_synthesis_api.py`, `test_migration_synthesis.py`) e 8 de frontend (`sintetizar.test.tsx`). 1618 testes de backend e 342 de frontend ao todo.
