...
| D-56 | Pilha de estágios de seleção multiestágio ordenada e heterogênea (P0-1) | aceito | abaixo |
| D-57 | Universo de processos como segundo universo de seleção e junção bidirecional (P0-2) | aceito | abaixo |
| D-58 | Seleção de processos como universo de saída do resultado (P0-3) | aceito | abaixo |
| D-59 | Atributos de processo com proveniência, envelope de capacidade e discreto (P0-4) | aceito | abaixo |
| D-60 | Chart Stage que filtra: regiões poligonais e linhas iso-índice como restrições (P1-2) | aceito | abaixo |
| D-61 | Browse taxonômico hierárquico, registro de família e ficha técnica de processos (P1-3) | aceito | abaixo |
| D-62 | My Records: registros próprios, favoritos e recentes isolados por usuário (P1-4) | aceito | abaixo |
| D-63 | Find Similar: distância log escalada, registro de referência e comparação percentual (P2) | aceito | abaixo |
| D-64 | Engineering Solver e Index Finder: dimensionamento estrutural e derivação analítica (P2) | aceito | abaixo |
| D-65 | Part Cost Estimator por parcelas de custo e objetivo custo nos casos de carga (P3) | aceito | abaixo |
...
## D-67 — Um registro sintetizado não é inventado: é calculado, carrega a receita inteira e vive em My Records

**Contexto.** O P3 da matriz do §5 de [`14-plataforma-selecao.md`](14-plataforma-selecao.md)
traz o *Synthesizer* (Módulo Q), o módulo que mais perto passa de "inventar
propriedades de materiais" — a proibição nº 1 da metodologia. Engenheiros precisam
explorar materiais hipotéticos (compósitos de dois constituintes com fração volumétrica,
espumas com densidade relativa, ou estruturas sanduíche). O sistema precisava admitir
registros derivados sem abrir mão da reprodutibilidade nem da integridade dos dados catalogados.

**Decisão.**

**1. Um valor sintetizado não é inventado; é calculado, e carrega a derivação.** O
princípio 1 proíbe número que veio do nada. Um limite de Voigt calculado a partir de
dois módulos catalogados e de uma fração declarada veio de um lugar verificável e
auditável. Para sustentar isso:
- O registro é explicitamente marcado como derivado (`material.is_synthesized = true`),
  e a receita completa (tipo, constituintes pais, parâmetros) é persistida em `material_synthesis`.
- Cada valor sintetizado nomeia a lei física que o gerou (`Rule.label`, `Rule.formula`)
  e sua base epistêmica (`exato`, `limites` ou `empirico`).
- A incerteza do modelo nunca vira barra de erro inventada: é declarada em prosa.

**2. A regra de mistura é propriedade da propriedade, não da receita.** Aplicar regra
das misturas indistintamente a qualquer número é a forma clássica pela qual ferramentas
de seleção mentem sem errar a aritmética. No Synthesizer:
- **Densidade** mistura linearmente por volume: conservação de massa estrita (`exato`).
- **Módulo de Young** em compósito depende da orientação; sem ela, a resposta honesta
  é o **par de limites** de Voigt (superior) e Reuss (inferior) (`limites`).
- **Grandezas por unidade de massa** (`custo_massa`, `energia_incorporada`, `pegada_co2`,
  reciclagem) misturam por **fração mássica**, não volumétrica, lendo a densidade de cada
  constituinte (`needs=(DENSIDADE,)`).
- **Temperatura máxima de serviço** é o **mínimo** dos constituintes: o compósito falha
  quando o constituinte mais vulnerável degrada.
- **Resistência mecânica não tem regra em compósito**: quem governa é a interface
  fibra-matriz, sobre a qual o catálogo não possui dados. Propriedades sem lei física
  honesta são **omitidas com o motivo técnico registrado** (`_NO_RULE`), cumprindo o
  Princípio 3 e D-24.

**3. Espumas obedecem à mecânica celular de Gibson–Ashby.** Uma espuma é o mesmo sólido
com poros: densidade escala com a densidade relativa $R = \rho^* / \rho_s$, módulo escala
com $R^2$, e colapso plástico escala com $0,3 \cdot R^{3/2} \cdot \sigma_s$ (empírico).
Propriedades por unidade de massa são herdadas do sólido original, pois a substância é a mesma.

**4. Propagação estrita da qualidade de dados.** Um valor sintetizado herda a **pior
qualidade** entre os valores dos pais que a regra leu (`MEDIDO` > `IMPORTADO` > `ESTIMADO`).
Um cálculo nunca é mais confiável do que o dado menos confiável que o alimentou.

**5. Registro derivado tem dono e nunca polui o catálogo compartilhado.** Um material
sintetizado é sempre propriedade do usuário (`owner_id NOT NULL`, assegurado no banco por
`ck_material_sintetizado_tem_dono`). O catálogo compartilhado contém apenas materiais de
referência reais ou de demonstração.

**6. Prévia antes de gravar.** `POST /api/sintetizar/previa` exibe os valores calculados,
as leis utilizadas e as propriedades omitidas com suas justificativas, permitindo validação
prévia antes da persistência no banco (`POST /api/sintetizar`).

**Como se verifica.** Testes em `test_synthesis.py` provam as leis físicas (conservação de
volume na densidade, fração mássica no custo, limites de Voigt-Reuss no módulo, Gibson-Ashby
na espuma, propagação da pior qualidade). Testes de API em `test_synthesis_api.py` provam
preview, isolamento por usuário e bloqueio de criação anônima. Teste de migração
`test_migration_synthesis.py` garante a transição do schema nos dois sentidos com dados pré-existentes.

**O que se recusou.** Mistura linear cega para todas as propriedades; inventar resistência
para compósitos sem caracterização de interface; permitir material sintetizado sem dono no
catálogo público; estimar custo do processo de espumação sem dado de processo.
